...
**One way implementation diverged from the spec.** The planning document's score aggregation section defined the confidence formula as a simple average of both signals — `(signal_1_score + signal_2_score) / 2` — treating them as equally weighted. During implementation this changed to a weighted average — `(0.6 × signal_1_score) + (0.4 × signal_2_score)` — giving the Groq LLM signal more influence than the stylometric signal. The reason: structural heuristics like sentence length variance and type-token ratio are surface-level proxies, while Signal 1 evaluates meaning and context directly. A text can have varied sentence lengths and dense punctuation while still reading as clearly AI-generated in substance. Giving Signal 1 a higher weight reflects that Signal 2 is a corroborating check, not an equal peer. The planning document was not updated to reflect this change before submission, leaving an inconsistency between the spec's aggregation section and the actual implementation.
